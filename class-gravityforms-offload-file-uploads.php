...
class GF_Offload_File_Uploads extends GFAddOn {
    ...
    public function init() {
        ...
        if ($plugin_options = get_option('gravityformsaddon_offload_file_uploads_settings')) {
            $this->s3serverurl = $plugin_options['s3serverurl'];
        ...
        return [
            [
                ...
                'fields' => [
                    [
                        'name'  => 's3serverurl',
                        'label' => esc_html__('Server URL', 'vital'),
                        'type'  => 'text',
                        'class' => 'medium',
                    ],
                    [
    ...
    public function upload_files_s3($files) {
        ...
        if ($this->awsaccesskey === ''
            ...
            error_log(sprintf(
                '%s: Required Amazon S3 credentials are missing in the plugin settings. Make sure you have set your AWS access key, secret key, S3 bucket name, and server url.',
                $this->_title,
            ));
        ...
        $remote_files = [];
        $s3 = new S3($this->awsaccesskey, $this->awssecretkey, true, $this->s3serverurl);
        // this is, if you don't have a bucket to create one
        ...
        foreach ($files as $key => $file) {
            ...
            $path = $this->encodePath($user->user_firstname . "-" . $user->user_lastname) . "/" . date("Y-m-d_H-i") . "/";

            if ($this->s3bucketpath) {
                $file_name = trailingslashit($this->s3bucketpath . $path) . $file_name;
            }
    ...
    public function encodePath(string $string)
    {
        $search = array("Ä", "Ö", "Ü", "ä", "ö", "ü", "ß", "´", "/", ">", "<", "\\");
        $replace = array("Ae", "Oe", "Ue", "ae", "oe", "ue", "ss", "-", "-", "-", "-", "-");
        return urlencode(strtolower(str_replace($search, $replace, $string)));
    }
}
